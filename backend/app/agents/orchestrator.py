...
- education: 用户上传了照片需要识别（暂归 medicine_photo）
- emergency: 头晕、摔倒、剧痛等紧急健康事件
- diary: 🆕 写日记、记心情、看心情趋势、回顾日记
- greeting: 普通的打招呼、闲聊"""
    ...
    async def process_message(
        ...
        elif intent == Intent.DIARY:
            result = await self.diary_agent.handle(user_input, ctx)
    ...
    async def _classify_intent(self, user_input: str, image_url: Optional[str] = None) -> Intent:
        ...
        # 🆕 规则先行：日记心情类
        diary_keywords = ["写日记", "记日记", "日记", "心情", "开心", "开心吗", "好开心",
                          "不高兴", "心情怎么样", "最近心情", "情绪", "心情趋势", "今天高兴",
                          "心情好", "好难过", "烦躁", "焦虑", "记下来", "回想", "记忆回顾", "那天"]
        if any(kw in user_input for kw in diary_keywords):
            return Intent.DIARY
